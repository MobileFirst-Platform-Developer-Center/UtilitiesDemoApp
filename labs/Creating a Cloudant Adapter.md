...
<property name="DBName" displayName="Cloudant DB name" defaultValue=""/>
```

This adds four fields to our console: Cloudant account, Cloudant password, Cloudant key, and Cloudant DB name. We will come back to this later when we set up out Cloudant instance and get these values. For now, we need to be able to access these values from our adapter code.

In our `CloudantUtilitiesApplication.java` file we need to add code to our `init()` method to connect to our database when the adapter starts. We'll be relying on the libraries `org.lightcouch.CouchDbException` to handle errors for us and `com.ibm.mfp.adapter.api.ConfigurationAPI` to access the variables we just added to the console. Make sure these are added to our `pom.xml` dependencies.
...
Now we can edit the `CloudantUtilitiesApplication.java` file.
...
The last step before we can move on is to call this Database varaible from out `CloudantUtilitiesResource.java` file. This time we'll be using the `com.ibm.mfp.adapter.api.AdaptersAPI` library to connect to the `CloudantUtilitiesApplication` class.
...
![Add the database](resources/Cloudant/database.png)
...
![Save the design document](resources/Cloudant/design_doc.png)
...
![Create the api key](resources/Cloudant/api_key.png)
...
![Input the credentials](resources/Cloudant/credentials.png)
...
![Test using swagger](resources/Cloudant/swagger.png)